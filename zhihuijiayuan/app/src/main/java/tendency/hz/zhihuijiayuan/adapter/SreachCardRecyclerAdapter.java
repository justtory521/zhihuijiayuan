...
public class SreachCardRecyclerAdapter extends RecyclerView.Adapter {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == What.RecyclerItemType.typeItem) {
            return new SreachRecyclerViewHolder(mInflater.inflate(R.layout.item_find_card_recycler, parent, false), mListener);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof SreachRecyclerViewHolder) {
            ((SreachRecyclerViewHolder) holder).mTextViewName.setText(mList.get(position).getTitle());
            ((SreachRecyclerViewHolder) holder).mImageCard.setImageURI(FormatUtils.getInstance().isEmpty(mList.get(position).getPoster()) ? mList.get(position).getPosterUrl() : mList.get(position).getPoster());
            ((SreachRecyclerViewHolder) holder).mTextViewSubTitle.setText(FormatUtils.getInstance().isEmpty(mList.get(position).getSubTitle()) ? "副标题" : mList.get(position).getSubTitle());
            ((SreachRecyclerViewHolder) holder).mTextViewCardTitle.setText(mList.get(position).getTitle());
            ((SreachRecyclerViewHolder) holder).mHeadImg.setImageURI(FormatUtils.getInstance().isEmpty(mList.get(position).getLogo()) ? mList.get(position).getLogoUrl() : mList.get(position).getLogo());
        }
    ...
    public class SreachRecyclerViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public SreachRecyclerViewHolder(View itemView, SearchCardItemOnClickInter findCardItemOnClickInter) {
            super(itemView);
